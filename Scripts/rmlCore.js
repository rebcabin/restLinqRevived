...
ObjectStore.prototype.Add = function(obj)
{
    ...
    if (!typeSet)
        // one Parametric list for each distinct type
        typeSet = this.byType[obj.type] = new Parametric(); 

    typeSet.update( obj, obj.priority );

    if (obj.transform)
    {
        if (obj.transform.worldPos)
            obj._local.coord = MercatorToNormalized(new Vector2(obj.transform.worldPos));
    ...
    if (obj.entities) // patch up entity links for relationships here
    {
        ...
        if (!rel._local)
            rel._local =
            { priotity: 1 - obj.priority,
              size: 1,
              animSpider: this.GetTime()
            };

        var found = this.Find(rel.sources.id);
        if (found && found._local.animPending)
            delete found._local.animPending; // remove the spinning animation
    }
